...
To trim the installable catalog, pass `--skip <id1[,id2,...]>` to install everything except the listed tools/apps, or `--pick <id1[,id2,...]>` to install only the listed ones. The two flags are mutually exclusive. Each id names one installable unit, and the recognized ids are listed (sorted) by `--help`; a value may be one or more comma-separated ids and the flag may be repeated. A `--pick` run is fully non-interactive: it skips the two up-front menus, and `--ide`/`--password-manager` are rejected alongside it. A `--skip` run still asks the menus unless you also pass those flags. Every listed unit is an independent application or command-line tool, so the flags only choose which units the run touches; Homebrew resolves each package's own dependencies. The base toolchain (Homebrew, Git, git identity and GitHub SSH, nvm/Node, yarn, and the shell setup) always installs, whatever the flags.

The script also installs the recommended IDE extensions into every IDE it finds on disk (VS Code and Cursor), independently of the `--ide` choice; leaving an IDE out via `--skip`/`--pick` also leaves that IDE's extension injection alone. Likewise, the 1Password CLI (`op`) installs independently of the `--password-manager` choice, which covers the GUI apps only; `op` itself is still a pickable/skippable unit.
