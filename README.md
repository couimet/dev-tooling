...
For an unattended run, pass `--ide <vscode|cursor|both|skip>` and `--password-manager <macpass|1password|both|skip>`; without the flags the script asks both questions up front, before it installs anything, so answering them leaves the rest of the run unattended.
